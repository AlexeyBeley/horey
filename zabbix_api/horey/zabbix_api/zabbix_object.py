...
class ZabbixObject:
        ...
        if isinstance(obj_src, dict):
            ...
            for key, value in obj_src.items():
                if type(key) not in [int, str]:
                    raise Exception
                ret[key] = ZabbixObject.convert_to_dict_static(value, custom_types=custom_types)
            return ret

        if isinstance(obj_src, list):
            return [ZabbixObject.convert_to_dict_static(value, custom_types=custom_types) for value in obj_src]

        if isinstance(obj_src, ZabbixObject):
            return obj_src.convert_to_dict()

        if isinstance(obj_src, datetime.datetime):
            return {ZabbixObject.SELF_CACHED_TYPE_KEY_NAME: "datetime", "value": obj_src.strftime("%Y-%m-%d %H:%M:%S.%f%z")}

        if isinstance(obj_src, Region):
            return {ZabbixObject.SELF_CACHED_TYPE_KEY_NAME: "region", "value": obj_src.convert_to_dict()}

        if isinstance(obj_src, IP):
            return {ZabbixObject.SELF_CACHED_TYPE_KEY_NAME: "ip", "value": obj_src.convert_to_dict()}
    ...
    def init_default_attr(self, attr_name, value, formatted_name=None):
        ...
        if formatted_name is None:
            formatted_name = self.common_re_utils.pascal_case_case_to_snake_case(attr_name)
