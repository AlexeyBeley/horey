...
configuration.authorization_map_file_path = os.path.join(
...
def test_authorization_applicator_serialize():
    restrictions = [{"name": "var_1", "value": "val_1", "required": True}]
    dict_src = {
        "job_name": "job_name",
...
def test_authorization_applicator_request_init_special_chars():
    request = AuthorizationApplicator.Request(
        '{"user_identity": "john.doe horey_special_char_replacement_lessjohn.doe@gmail.comhorey_special_char_replacement_more", "job_name": "job_name", "parameters": {"var_1": "val_1"}}'
    )
    assert request.job_name == "job_name"
    assert request.parameters == {"var_1": "val_1"}
...
def test_authorization_applicator_request_serialize():
    ...
    assert json.loads(request.serialize()) == {
        "job_name": "job_name",
        "parameters": {"var_1": "val_1"},
        "user_identity": "user_identity",
    }
