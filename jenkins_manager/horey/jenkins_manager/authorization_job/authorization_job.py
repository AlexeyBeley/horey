...
class AuthorizationJob:
    ...
    def __init__(self, configuration):
        ...
        self.authorization_applicator.deserialize(
            configuration.authorization_map_file_path
        )
    ...
    def authorize(self, request: AuthorizationApplicator.Request):
        ...
        if not self.authorization_applicator.authorize(request):
            raise RuntimeError(
                f"User {request.user_identity} unauthorized to run {request.job_name}, with params {request.parameters}"
            )
