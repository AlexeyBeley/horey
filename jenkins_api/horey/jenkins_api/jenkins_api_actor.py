...
def run_jobs_parser():
    ...
    parser = argparse.ArgumentParser(description=description)
    parser.add_argument(
        "--cached_jobs_file_path",
        required=True,
        type=str,
        help="Json list of cached jobs",
    )
...
def run_jobs(arguments, configs_dict) -> None:
    ...
    with open(arguments.cached_jobs_file_path, encoding="utf-8") as file_handler:
        lst_dict_jobs = json.load(file_handler)
    lst_jobs = [
        JenkinsJob(None, None, cache_dict=dict_job) for dict_job in lst_dict_jobs
    ]
...
def backup_jobs_parser():
    ...
    parser.add_argument(
        "--jobs_names", required=False, type=str, help="job_names", default=""
    )
...
def create_jobs_parser():
    ...
    parser.add_argument(
        "--overwrite", required=True, type=str, help="overwrite existing"
    )
...
def disable_jobs_parser():
    usage_example = (
        "jenkins_api_actor.py --action disable_jobs --input_file ${ROOT_DIR}/jenkins_disable.txt"
...
def delete_jobs_parser():
    usage_example = (
        "jenkins_api_actor.py --action delete_jobs --input_file ${ROOT_DIR}/jenkins_disable.txt"
        " --configuration_file_full_path ${ROOT_DIR}/jenkins_configuration_file.py"
    )
...
def find_build_parser():
    ...
    parser = argparse.ArgumentParser(description=description)
    parser.add_argument(
        "--jobs_names", required=True, type=str, help="comma separated jobs' names"
    )
    parser.add_argument(
        "--search_string", required=True, type=str, help="string to search for"
    )
