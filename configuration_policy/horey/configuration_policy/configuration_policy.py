...
class ConfigurationPolicy:
    ...
    def init_from_environ(self):
        """
        Load attributes values from environment variables.
    ...
    def init_from_json_file(self):
        ...
        with open(self.configuration_file_full_path, encoding="utf-8") as file_handler:
    ...
    def init_from_policy(self, configuration, ignore_undefined=False):
        ...
        for attr_name, value in configuration.convert_to_dict(
            ...
            try:
                ...
                logger.info(log_line)
            except (AttributeError, self.UndefinedValueError):
                if not ignore_undefined:
                    raise
    ...
    def validate_type_decorator(types):
        """
        Validate value for a property setter.

        :param types: possible types for a value
        :return:
        """
        def function_receiver(func):
            """
            This is needed to make operation on a property
    ...
    class StaticValueError(RuntimeError):
        """
        Suppressing is not allowed for the property.
