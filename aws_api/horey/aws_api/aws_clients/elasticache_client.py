...
class ElasticacheClient(Boto3Client):
    ...
    def get_region_cache_security_groups(self, region, cache_security_group_name=None):
        ...
        final_result = list()
        filters_req = {"CacheSecurityGroupName": cache_security_group_name} if cache_security_group_name is not None else None
        for dict_src in self.execute(self.client.describe_cache_security_groups, "CacheSecurityGroups", filters_req=filters_req):
    ...
    def provision_subnet_group(self, subnet_group):
        ...
        response = self.provision_subnet_group_raw(subnet_group.generate_create_request())
    ...
    def provision_subnet_group_raw(self, request_dict):
        ...
        for response in self.execute(self.client.create_cache_subnet_group, "CacheSubnetGroup",
    ...
    def provision_cluster_raw(self, request_dict):
        ...
        for response in self.execute(self.client.create_cache_cluster, "CacheCluster",
                                     filters_req=request_dict):
            return response
        
    def provision_replication_group(self, replication_group):
        region_replication_groups = self.get_region_replication_groups(replication_group.region)
        for region_replication_group in region_replication_groups:
            if replication_group.id == region_replication_group.id:
                replication_group.update_from_raw_response(region_replication_group.dict_src)
                return

        AWSAccount.set_aws_region(replication_group.region)
        response = self.provision_replication_group_raw(replication_group.generate_create_request())
    ...
    def provision_replication_group_raw(self, request_dict):
        ...
        for response in self.execute(self.client.create_replication_group, "ReplicationGroup",
                                     filters_req=request_dict):
            return response
        
    def provision_security_group(self, security_group):
        pdb.set_trace()
        region_security_groups = self.get_region_cache_security_groups(security_group.region,
                                                                      cache_security_group_name=security_group.name)
        if len(region_security_groups) > 0:
            region_security_group = region_security_groups[0]
            request = security_group.generate_authorize_request(existing_security_group=region_security_group)
            if request:
        ...
        response = self.provision_security_group_raw(security_group.generate_create_request())
    ...
    def provision_security_group_raw(self, request_dict):
        ...
        for response in self.execute(self.client.create_cache_security_group, "CachesecurityGroup",
                                     filters_req=request_dict):
            return response
    ...
    def authorize_security_group_raw(self, request_dict):
        ...
        for response in self.execute(self.client.authorize_cache_security_group_ingress, "CacheSecurityGroup",
                                     filters_req=request_dict):
            return response
