...
class ServicediscoveryClient(Boto3Client):
    ...
    def get_region_services(self, region, custom_filters=None):
        ...
        for response in self.execute(self.client.list_services, "Services", filters_req=custom_filters):
    ...
    def get_region_namespaces(self, region, full_information=False, custom_filters=None):
        ...
        for response in self.execute(self.client.list_namespaces, "Namespaces", filters_req=custom_filters):
    ...
    def update_namespace_information(self, namespace):
        custom_filters = [{"Name": "TYPE", "Values": [namespace.type], "Condition": "EQ"}]
        region_namespaces = self.get_region_namespaces(namespace.region, custom_filters=custom_filters)
    ...
    def update_service_information(self, service):
        if service.namespace_id is None:
            raise RuntimeError(f"Can not find service without namespace_id been set: {service.name}")

        custom_filters = [{"Name": "NAMESPACE_ID", "Values": [service.namespace_id], "Condition": "EQ"}]

        region_services = self.get_region_services(service.region, custom_filters=custom_filters)
    ...
    def provision_service(self, service: ServicediscoveryService):
        ...
        try:
            ...
            logger.warning(f"{service.generate_create_request()}: {repr(exception_inst)}")
            raise

    def provision_service_raw(self, request):
        for response in self.execute(self.client.create_service, "Service", filters_req=request):
            return response
