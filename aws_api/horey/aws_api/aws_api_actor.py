...
def cleanup(arguments) -> None:
    ...
    output_files = {
        "interfaces": configuration.aws_api_cleanups_network_interfaces_report_file
    }

    init_functions[arguments.target](
        from_cache=True, cache_file=cache_files[arguments.target]
    )
...
action_manager.register_action(
    "create_ec2_from_lambda", create_ec2_from_lambda_parser, create_ec2_from_lambda
)
# endregion
...
action_manager.register_action(
    "put_secret_file", put_secret_file_parser, put_secret_file
)
# endregion
...
action_manager.register_action(
    "put_secret_value", put_secret_value_parser, put_secret_value
)
# endregion
...
def get_secret_file(arguments) -> None:
    if arguments.configuration_file_full_path != "None":
        ...
        configuration.configuration_file_full_path = (
            arguments.configuration_file_full_path
        )
        configuration.init_from_file()
        aws_api = AWSAPI(configuration)
    else:
...
action_manager.register_action(
    "get_secret_file", get_secret_file_parser, get_secret_file
)
# endregion
