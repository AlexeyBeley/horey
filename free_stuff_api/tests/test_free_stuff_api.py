...
def fixture_free_stuff_mac_raw():
    auction_api = FreeStuffAPI(config)
    yield auction_api
    auction_api.selenium_api.disconnect()
...
def test_add_free_item_to_db():
    ...
    assert auction_api.add_free_item_to_db(free_item)
...
def test_notify_about_new_item():
    ...
    assert auction_api.notify_about_new_item(free_item)

@pytest.mark.unit
def test_provision_infra():
    auction_api = FreeStuffAPI(config)
    assert auction_api.provision_infra()


@pytest.mark.unit
def test_update():
    auction_api = FreeStuffAPI(config)
    assert auction_api.update()
