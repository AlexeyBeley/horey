...
class MachineDeploymentStep:
    ...
    def update_finish_status(self, local_deployment_data_dir_path):
        try:
            ...
            self.status_code = MachineDeploymentStep.StatusCode.ERROR
            self.status = f"File '{os.path.join(local_deployment_data_dir_path, self.configuration.finish_status_file_name)}' not found"
            return

        self.status = status
        try:
            self.status_code = MachineDeploymentStep.StatusCode.__members__[status]
        except KeyError:
            self.status_code = MachineDeploymentStep.StatusCode.ERROR
